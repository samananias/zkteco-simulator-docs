...
| Camera touchless capture (zero hardware) | **FF for demo-grade quality** (quality bar documented, not hidden) | FR-16 | 7 | ✅ core (2026-09-16): `/enroll` + `/scan` PWA, capture endpoint + quality gate (actionable reasons), ceremony, biometric punch → EF_ATTLOG/BITS — automated tests green; **remaining:** owner-run live-camera UX check + human-judged quality assessment recorded |
| Android + USB-OTG scanner capture | **DE** `[A]` — depends on a purchased module + integration validation | FR-16 | future | ⬜ **deferred by owner decision 2026-09-16** (allowance-gated, moved out of Phase 7); design preserved in capture-stations §3 — plugs into the same capture contract with zero core changes when funded |
